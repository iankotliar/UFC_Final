...
3. **Exploratory Data Analysis & Historical Profitability Analysis**
    ...
    * I found that simply taking the average of the "balanced book" probabilities from each betting site produced a very good estimate of the true probability of a fighter winning. The reliability plot fit almost perfectly on a 45 degree line, and a regression of proportion of fighters won on average probability with 5% probability buckets had an R squared close to .99 with an intercept close to 0 and coefficient close to 1. This suggests that the UFC betting market is extremely efficient. 
    ...
    * Because the average of the balanced book probabilities is such an accurate predictor of fight outcomes, I wondered if I could use it to detect betting opportunities with positve expected value from sites offering mispriced odds. However, I found that the probabilities accross sites tended to be very close to the average, and after accounting for the vig only very marginal opportunities seemed to exist. A strategy based on only placing bets with a calculated expected return greater than a selected threshold (e.g. 2%, 3%, 4%) did make money in most years, but did not produce a worthwhile risk-reward balance. This is not surprising, since there were almost no opportunities with a calculated expected return greater than 4%.  
4. **Future Work**
